...
(function () {
  ...
  function ensureFirebase() {
    ...
    return loadScript('https://www.gstatic.com/firebasejs/10.12.2/firebase-app-compat.js')
      .then(function(){ return loadScript('assets/firebase-init.js?v=6'); })
  ...
  function buildMenu(acceso) {
    ...
    if (conSesion) {
      ...
      if (header && !document.getElementById('sessionBadge')) {
        ...
        b.id = 'sessionBadge';
        b.textContent = acceso.admin ? 'Sesión: Admin' : (acceso.teacher ? 'Sesión: Docente' : 'Sesión: Equipo AVI');
  ...
  function start() {
    ...
    ensureFirebase().then(function () {
      firebase.auth().onAuthStateChanged(function (user) {
        ...
        window.AVI_privateAccess(user).then(function (acceso) { buildMenu(acceso); });
      });
